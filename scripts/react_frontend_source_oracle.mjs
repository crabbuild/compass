...
function runTypescriptOracle(root) {
  ...
  return { records, header, footer, raw: Buffer.from(result.stdout, "utf8") };
}
...
function declarationTargets(source, sourceFile) {
  ...
  const renderableDeclaration = (node) => {
    ...
    if (ts.isTaggedTemplateExpression(initializer)) return true;
    if (ts.isCallExpression(initializer)) {
      const callee = unwrapExpression(initializer.expression);
      // Context values are uppercase in common React code but are not
      // component targets. Keep the exclusion narrow; wrappers such as
      // `withSpan(...)`, `createDataFetcher(...)`, and component factories
      // are valid renderable aliases in the graph contract.
    ...
    // Aliases such as `const Popover = Primitive.Root` and conditional
    // component selections are valid JSX targets even when their initializer
    // is not itself a function expression.
    return true;
  };
...
function sourceFacts(root, framework, records) {
  ...
  const routeParent = (file, candidates) => {
    ...
    for (let index = parts.length - 1; index > 0; index -= 1) {
      ...
      const parent = candidates.find((candidate) => candidate.split("/").slice(0, -1).join("/") === parentDirectory);
  ...
  for (const [file, entry] of astFiles) {
    ...
    if (framework === "next-app" && /(^|\/)app\/(?:.+\/)?(page|layout|template|error|global-error|loading|not-found|default|route)\.[cm]?[jt]sx?$/u.test(file)) {
    ...
    if (framework === "next-pages" && /(^|\/)pages\/.+\.[cm]?[jt]sx?$/u.test(file)) {
      routeFiles.push(file);
      const resolved = defaultRouteTarget(file, source, ast, targets);
      const target = resolved?.target ?? null;
