...
def run_checked(command: list[str], *, cwd: Path) -> subprocess.CompletedProcess[str]:
    ...
    return subprocess.run(command, cwd=cwd, env=qualification_env(), capture_output=True, text=True, check=False)
...
def load_manifest(path: Path) -> dict[str, Any]:
    ...
    for key in ("minimumPrecision", "minimumRecall", "minimumWilsonLower95", "maxColdRegression", "maxWarmRegression", "maxPeakRssRegression"):
...
def run_compass(binary: Path, source: Path, output: Path, *, label: str, force: bool = False, workers: int | None = None) -> dict[str, Any]:
    ...
    with log_path.open("w", encoding="utf-8") as log:
        checked_command(command)
        process = subprocess.Popen(command, cwd=ROOT, env=qualification_env(), stdout=log, stderr=subprocess.STDOUT, text=True)
    ...
    return {"label": label, "graph": graph, "graphSha256": digest_file(graph), "seconds": elapsed, "peakRssBytes": process_rss_bytes(), "command": command, "returncode": returncode, "log": str(log_path)}
...
def run_interrupted(binary: Path, source: Path, output: Path, *, expected_digest: str) -> dict[str, Any]:
    ...
    return {"command": command, "signal": "SIGINT", "signalSent": signal_sent, "returncode": returncode, "seconds": time.perf_counter() - started, "peakRssBytes": process_rss_bytes(), "pointerExists": pointer_exists, "publishedDigest": published_digest, "partialArtifact": partial_artifact, "cancellationObserved": bool(signal_sent and returncode != 0), "log": str(log_path)}
...
def performance_comparison(result: dict[str, Any], baseline_path: Path | None) -> dict[str, Any]:
    ...
    for report in result["repositories"]:
        ...
        for row in report.get("performance", {}).get("rows", []):
            ...
            seconds_ratio = row["seconds"] / old["seconds"] if old.get("seconds", 0) > 0 else 0.0
            rss_ratio = row["peakRssBytes"] / old["peakRssBytes"] if old.get("peakRssBytes", 0) > 0 else 0.0
...
def run(args: argparse.Namespace) -> dict[str, Any]:
    ...
    for repository in manifest["repository"]:
        ...
        original = semantic_file.read_bytes()
        ...
        config_file.write_bytes(config_file.read_bytes() + b"\n")
        ...
        report = {
            ...
            "performance": {"rows": [{key: observation[key] for key in ("label", "seconds", "peakRssBytes", "graphSha256", "command")} for observation in (cold, warm, semantic, manifest_edit, restored, alternate)], "semanticEditRestored": restored["graphSha256"] == cold["graphSha256"]},
