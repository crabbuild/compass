...
export const metadata: Metadata = {
  title: 'Blog',
  description: 'Notes on code graphs, provenance, local-first tooling, and building Compass.',
};
  ...
  return (
    <>
      <section className="relative overflow-hidden border-b border-border/70">
        ...
        <div className="relative mx-auto grid max-w-7xl gap-10 px-5 pb-16 pt-20 lg:grid-cols-[1fr_0.7fr] lg:items-end lg:px-8 lg:pb-24 lg:pt-28">
          <div>
            ...
            <h1 className="mt-5 max-w-4xl font-heading text-[clamp(3rem,7vw,6.4rem)] font-semibold leading-[0.94] tracking-[-0.075em]">Field notes for complex systems.</h1>
            <p className="mt-7 max-w-2xl text-lg leading-8 text-muted-foreground">Launch notes, implementation stories, and practical ways to make a codebase easier to navigate—one inspectable relationship at a time.</p>
          </div>
          <div className="grid grid-cols-2 gap-3 sm:grid-cols-3 lg:grid-cols-2">
            <BlogStat value={String(posts.length).padStart(2, '0')} label="launch notes" />
      ...
      <section className="mx-auto max-w-7xl px-5 py-16 lg:px-8 lg:py-24">
        ...
        ) : <Card className="border-border/80 bg-card/70 shadow-none"><CardContent className="p-8 text-sm text-muted-foreground">New notes are being prepared.</CardContent></Card>}
      </section>
      {rest.length > 0 && <section className="border-y border-border/70 bg-muted/25"><div className="mx-auto max-w-7xl px-5 py-16 lg:px-8 lg:py-24"><div className="flex items-end justify-between gap-6"><div><p className="eyebrow">More notes</p><h2 className="mt-3 font-heading text-3xl font-semibold tracking-[-0.05em]">Build a better mental model.</h2></div><span className="hidden font-mono text-xs text-muted-foreground sm:block">{rest.length} more {rest.length === 1 ? 'note' : 'notes'}</span></div><div className="mt-10 grid gap-5 md:grid-cols-2">{rest.map((post) => <BlogCard key={post.url} post={post} />)}</div></div></section>}
...
function FeaturedPost({ post }: { post: ReturnType<typeof blogSource.getPages>[number] }) {
  return <Card className="overflow-hidden border-border/80 bg-card/70 shadow-none"><CardHeader className="gap-6 border-b border-border/70 p-6 sm:p-8"><div className="flex flex-wrap items-center justify-between gap-3"><Badge>Launch note</Badge><span className="flex items-center gap-1.5 font-mono text-xs text-muted-foreground"><CalendarDaysIcon /> {formatDate(post.data.date)}</span></div><CardTitle className="max-w-3xl font-heading text-3xl tracking-[-0.05em] sm:text-4xl">{post.data.title}</CardTitle></CardHeader><CardContent className="flex flex-col gap-7 p-6 sm:p-8"><p className="max-w-2xl text-base leading-8 text-muted-foreground">{post.data.description}</p><div className="flex flex-wrap items-center gap-4"><Link className="inline-flex items-center gap-2 text-sm font-medium text-primary" href={post.url}>Read the launch note <ArrowRightIcon data-icon="inline-end" /></Link><span className="font-mono text-xs text-muted-foreground">{post.data.author}</span></div></CardContent></Card>;
}
...
function BlogCard({ post }: { post: ReturnType<typeof blogSource.getPages>[number] }) {
  return <Card className="group border-border/80 bg-card/70 shadow-none transition-transform duration-300 hover:-translate-y-1"><CardHeader className="gap-4"><div className="flex items-center justify-between gap-3"><Badge variant="outline">{post.data.tags[0] ?? 'Compass'}</Badge><span className="flex items-center gap-1.5 font-mono text-xs text-muted-foreground"><CalendarDaysIcon /> {formatDate(post.data.date)}</span></div><CardTitle className="max-w-lg font-heading text-2xl tracking-[-0.045em]">{post.data.title}</CardTitle></CardHeader><CardContent className="flex flex-col gap-6"><p className="text-sm leading-7 text-muted-foreground">{post.data.description}</p><Link className="inline-flex items-center gap-2 text-sm font-medium text-primary transition-[gap] group-hover:gap-3" href={post.url}>Read note <ArrowRightIcon data-icon="inline-end" /></Link></CardContent></Card>;
}
