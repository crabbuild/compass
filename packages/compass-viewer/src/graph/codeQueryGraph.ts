...
export function codeQueryGraphViewModel(
  ...
  const distances = pathDistances(result);
  const roots = new Set(
    result.paths.flatMap((path) => path.nodeIds.slice(0, 1))
  );
...
function confidence(edge: CodeQueryEdge): GraphEdge["confidence"] {
...
function pathDistances(result: CodeQueryResponse): Map<string, number> {
  const distances = new Map<string, number>();
  for (const path of result.paths) {
    path.nodeIds.forEach((nodeId, index) => {
      const current = distances.get(nodeId);
      if (current === undefined || index < current) distances.set(nodeId, index);
    });
  }
  return distances;
}
