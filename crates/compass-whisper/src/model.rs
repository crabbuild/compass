...
impl WhisperModel {
    ...
    pub fn load<P: AsRef<Path>>(config_path: P, weights_path: P, device: &Device) -> Result<Self> {
        ...
        // Buffered loading avoids exposing Candle's memory-mapping safety
        // preconditions outside the isolated tree-sitter FFI boundary.
        let weights = std::fs::read(weights_path.as_ref()).context("reading model weights")?;
...
mod tests {
    ...
    #[test]
    fn buffered_model_loader_fails_safely_for_missing_and_malformed_weights() -> Result<()> {
        let config_path = unique_temp_file("buffered-config.json");
        let weights_path = unique_temp_file("buffered-weights.safetensors");
        std::fs::write(
            &config_path,
            br#"{"num_mel_bins":2,"max_source_positions":4,"d_model":4,"encoder_attention_heads":2,"encoder_layers":1,"vocab_size":51864,"max_target_positions":16,"decoder_attention_heads":2,"decoder_layers":2,"suppress_tokens":[]}"#,
        )?;

        let Err(missing) = WhisperModel::load(&config_path, &weights_path, &Device::Cpu) else {
            return Err(anyhow::anyhow!("missing weights unexpectedly loaded"));
        };
        assert!(missing.to_string().contains("reading model weights"));

        std::fs::write(&weights_path, b"not-a-safetensors-document")?;
        assert!(WhisperModel::load(&config_path, &weights_path, &Device::Cpu).is_err());
