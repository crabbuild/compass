...
use trail_files::{DetectOptions, FileType, WatchPathFilter, classify_file, write_text_atomic};
use trail_languages::Registry;
...
pub fn watch_local_graph(
    ...
    let root =
        fs::canonicalize(&options.build.root).map_err(|source| trail_files::FileError::Io {
...
mod tests {
    ...
    fn watch_rebuilds_code_and_flags_semantic_changes() -> Result<(), Box<dyn Error>> {
        ...
        let graph = trail_model::GraphDocument::load(&root.join("graphify-out/graph.json"))?;
        assert!(
            graph
                .nodes
                .iter()
