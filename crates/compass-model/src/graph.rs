...
impl Graph {
    ...
    pub fn successors(&self, node: NodeIndex) -> impl Iterator<Item = NodeIndex> + '_ {
        ...
        self.outgoing_edges(node)
            ...
            .filter(move |neighbor| seen.insert(*neighbor))
    }
...
mod tests {
    ...
    #[test]
    fn multigraph_neighbors_are_unique_but_degree_counts_parallel_edges() {
        let raw = r#"{
            "directed": false,
            "multigraph": true,
            "nodes": [{"id":"a"},{"id":"b"}],
            "links": [
                {"source":"a","target":"b","relation":"one"},
                {"source":"a","target":"b","relation":"two"}
            ]
        }"#;
        let document: GraphDocument = serde_json::from_str(raw).unwrap_or_else(|_| {
            std::process::abort();
        });
        let graph = Graph::from_document(document).unwrap_or_else(|_| std::process::abort());
