...
pub fn resolve_language_calls(extractions: &[Extraction], merged: &mut Extraction) {
    ...
    resolve_typed_members(&calls, &indexes, &tables, &mut existing, &mut merged.edges);
    resolve_python_members(&calls, &indexes, &mut existing, &mut merged.edges);
    resolve_ruby_members(&calls, &indexes, &mut existing, &mut merged.edges);
    resolve_pascal_inherited(&calls, &indexes, &mut existing, &mut merged.edges);
}
