...
pub fn render_lessons_markdown(aggregate: &Aggregate) -> String {
    ...
    let mut output = vec![
        ...
        String::new(),
        format!(
            "_Auto-generated by `graphify reflect` from {} session {memories} in compass-out/memory/. Deterministic; no LLM. Use for orientation — verify before relying, and revisit dead ends if the code has changed since._",
            aggregate.total
        ),
        String::new(),
        "## Summary".to_owned(),
