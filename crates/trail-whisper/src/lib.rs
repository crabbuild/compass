//! Bounded native Whisper inference for Trail.
//!
...
//! Trail deliberately omits its network, file decoding, and output-writing
//! layers so those boundaries remain governed by Trail's own limits.
...
pub fn device(name: &str) -> Result<Device> {
    match name {
        "cpu" => Ok(Device::Cpu),
        other => anyhow::bail!("device {other} is not available; Trail currently supports CPU"),
    }
}
...
mod tests {
    ...
    fn bundled_english_tokenizer_round_trips_text() -> Result<()> {
        ...
        let text = " Trail maps code into a knowledge graph.";
