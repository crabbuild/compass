...
pub fn publish_resolved_routes(
    ...
    for (child_id, child) in &route_ids {
        ...
        if let Some(candidates) = route_sources_by_scope.get(&(child.framework.clone(), scope)) {
            if let Some(parent_source) =
                route_parent_source_file(&child.anchor.source_file, candidates)
            {
...
fn route_parent_source_file(
    ...
    for index in (1..parts.len()).rev() {
        ...
        if let Some((source, _, _, _)) = candidates.iter().find(|(source, _, _, _)| {
            if source.replace('\\', "/") == portable {
                return false;
            }
            let source = source.replace('\\', "/");
            source
                .rsplit_once('/')
                .map_or(source.is_empty(), |(directory, _)| {
