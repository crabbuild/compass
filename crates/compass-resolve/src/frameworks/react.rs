...
pub(crate) fn project_render_relations(extraction: &mut Extraction) {
    ...
    // JSX is itself a direct semantic use of the target.  A target may not
    // have received a separate `ui_component` role (for example a context
    // object rendered through a local alias), but an exact, local JSX
    // reference is still safe to project.  Keep this set just as strict as
    // the render loop below: local node kinds only, with no deferred or
