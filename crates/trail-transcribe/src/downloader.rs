...
const TOOL_USER_AGENT: &str = "trail/0.1 tool-fetch";
...
pub enum DownloaderError {
    ...
    UnsupportedPlatform,
    #[error("could not determine Trail's tool cache directory")]
...
impl ToolCache {
    ...
    pub fn from_environment() -> Result<Self, DownloaderError> {
        if let Some(root) = std::env::var_os("TRAIL_CACHE_DIR") {
            return Ok(Self::new(PathBuf::from(root).join("tools")));
        }
        if let Some(root) = std::env::var_os("XDG_CACHE_HOME") {
            return Ok(Self::new(PathBuf::from(root).join("trail/tools")));
        }
        if cfg!(windows)
        ...
        std::env::var_os("HOME")
            .map(PathBuf::from)
            .map(|root| Self::new(root.join(".cache/trail/tools")))
