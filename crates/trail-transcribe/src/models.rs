...
const USER_AGENT: &str = "trail/0.1 model-fetch";
...
pub enum ModelError {
    ...
    #[error("could not determine Trail's model cache directory")]
...
impl ModelCache {
    ...
    pub fn from_environment() -> Result<Self, ModelError> {
        ...
        if let Some(root) = std::env::var_os("XDG_CACHE_HOME") {
            return Ok(Self::new(PathBuf::from(root).join("trail/models")));
        }
        if cfg!(windows)
        ...
        std::env::var_os("HOME")
            .map(PathBuf::from)
            .map(|root| Self::new(root.join(".cache/trail/models")))
