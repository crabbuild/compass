...
use trail_transcribe::AudioDownloader;
...
fn verified_yt_dlp_downloads_decodable_audio() -> Result<(), Box<dyn Error>> {
    let url = std::env::var("TRAIL_YTDLP_ACCEPTANCE_URL")
        .map_err(|_| "set TRAIL_YTDLP_ACCEPTANCE_URL to a public audio or video URL")?;
