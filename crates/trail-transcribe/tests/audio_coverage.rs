...
use trail_transcribe::audio::{AudioLimits, WHISPER_SAMPLE_RATE, decode_audio, resample};
...
{
    let prompt = build_whisper_prompt(["Trail", "Graph"]);
    assert!(prompt.contains("Trail, Graph"));
    let directory = tempfile::tempdir()?;
