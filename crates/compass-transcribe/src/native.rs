//! Native Candle Whisper backend over Compass's bounded media and model layers.
...
use compass_whisper::{TranscribeOptions, WhisperModel};
...
impl NativeWhisperBackend {
    pub fn load(
        ...
        let device = compass_whisper::device("cpu").map_err(|error| error.to_string())?;
...
impl WhisperBackend for NativeWhisperBackend {
    fn transcribe(&mut self, request: &BackendRequest<'_>) -> Result<BackendTranscript, String> {
        ...
        let result = compass_whisper::transcribe(&mut self.model, &decoded.samples, &options)
            .map_err(|error| error.to_string())?;
...
mod tests {
    ...
    fn native_options_match_faster_whisper_request_contract() -> Result<(), String> {
        let request = BackendRequest {
            ...
            model: "base",
            beam_size: 5,
            initial_prompt: "Technical discussion about Compass.",
        };
        let options = native_options(&request)?;
        assert_eq!(
