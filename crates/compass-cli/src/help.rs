...
const PAGES: &[Page] = &[
    ...
    page!(
        ...
        "Arguments:\n  <SYMBOL>                      Symbol ID, name, or qualified name\n\nOptions:\n  --graph <PATH>                Typed graph [default: compass-out/graph.json]\n  --program <PATH>              Optional Program IR enrichment\n  --cache <DIR>                 Query-index cache directory\n  --engine <default|json|store> Graph storage engine [default: default]\n  --max-nodes <N>               Node bound\n  --max-edges <N>               Edge bound\n  --include-heuristic           Include heuristic evidence (default is exact-first)\n  --format <text|agent-json|json> Output format [default: text]\n  --text-budget <N>             Approximate tokens per text page [default: 2000]\n  --cursor <TOKEN>              Continue the same text result (text only)\n  --timeout-ms <N>              Query deadline in milliseconds [default: 60000; max: 600000]\n  --brief                       Emit compass.query.agent-view.brief/1 (agent-json only)\n\nExamples:\n  compass callers PaymentService.charge\n  compass callers sym:checkout --format json\n  compass callers PaymentService.charge --text-budget 800\n\nNotes:\n  Incoming usages include calls, routes, references, imports, exports, and aliases; each result retains its exact relationship kind. Text output is paged: the footer carries a cursor that continues the same deterministic result at the same page budget."
    ),
    page!(
        "callees",
    ...
    page!(
        ...
        "Options:\n  --graph <PATH>          Graph JSON [default: compass-out/graph.json]\n  --labels <PATH>         Community-label JSON\n  --node-limit <N>        Maximum nodes rendered [default: 5000]\n  --hierarchy-level <N>   Community level the page opens on\n  --no-viz                Skip visualization output\n\nExamples:\n  compass export html\n  compass export html --node-limit 2000\n  compass export html --hierarchy-level 1\n\nNotes:\n  Large exports embed a bounded detail for every community, most connected symbols first; the viewer names the window and points at VS Code or export json --community ID for the complete community. A page built from a clustered graph also embeds the published community hierarchy and opens on the coarsest level that decomposes the repository, or on the level --hierarchy-level names; a hierarchy whose every level holds one group leaves the reader on the symbol canvas with every published level still selectable, and an unknown level or a missing hierarchy fails instead of rendering a different page. Source actions open immutable links for recognized GitHub, GitLab, and Bitbucket origins. Compass uses the graph commit when published, or a published ancestor only when every represented source file is byte-identical; otherwise the viewer explains that local navigation requires VS Code. Interactive terminals ask before opening the generated HTML; scripts and --no-viz never prompt or open a browser."
    ),
    page!(
