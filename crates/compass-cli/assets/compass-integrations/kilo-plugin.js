...
const server = async ({ directory }) => {
  ...
  return {
    "tool.execute.before": async (input, output) => {
      ...
      if (input.tool === "bash") {
        output.args.command =
          'echo "[compass] Focused task: query first. Broad first session: read only Agent Orientation at the start of GRAPH_REPORT.md, then query. Read RESULT, ANSWER, and CAVEATS before details; inspect direction, ambiguity, completeness, domain truncation, pagination, and minimal cited source. Keep compass watch running or update after edits." ; ' +
          output.args.command;
        reminded = true;
      }
    },
...
export default { id: "compass", server };
