...
-> Result<(), Box<dyn Error>> {
    ...
    let arxiv_result = ingest_with(
        ...
        &Fixtures(HashMap::from([(
            arxiv_api.to_owned(),
            Ok(br#"<h1 class="title mathjax">Title: <span>Compass Graphs</span></h1><div class="authors"><a>Ada</a>, <a>Lin</a></div><blockquote class="abstract mathjax">Abstract: <b>Fast</b> graphs.</blockquote>"#.to_vec()),
        )])),
    )?;
    let arxiv_text = fs::read_to_string(arxiv_result.path)?;
    assert!(arxiv_text.contains("# Title:"));
    assert!(arxiv_text.contains("Compass Graphs"));
