...
impl<'source, 'tree> ExtractState<'source, 'tree> {
    ...
    fn walk_python_import_calls(&mut self, node: Node<'tree>, caller: &str) {
        if node.kind() == "call"
            ...
            && let Some(local_name) = self.node_text(function).map(clean_name)
            && let Some(name) = self.python_import_aliases.get(&local_name).cloned()
        {
    ...
    fn call_name(&self, node: Node<'tree>) -> Option<CallName> {
        ...
        let receiver =
            if member && self.language != "python" && !self.config.accessor_object_field.is_empty()
            {
                function
...
fn python_import_aliases(root: Node<'_>, source: &[u8]) -> HashMap<String, String> {
    fn walk(node: Node<'_>, source: &[u8], output: &mut HashMap<String, String>) {
