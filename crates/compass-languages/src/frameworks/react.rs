...
pub(super) fn detect(context: &UniversalDetectionContext<'_, '_>) -> Vec<RawFrameworkFact> {
    ...
    for declaration in &context.evidence.declarations {
        ...
        // A Next/React client module establishes a client boundary for every
        // component declared in the module.  Only declarations exported
        // through that boundary receive the stronger client-component role.
        if has_client_directive && is_component {
            roles.insert("client_boundary");
            if node.is_some_and(is_exported_declaration) {
