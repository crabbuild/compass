...
pub(super) fn detect_next(
    ...
    if router == "app" {
        if let Some(app_file) = next_app_file(relative) {
            ...
            let reference =
                next_default_export_reference(syntax).unwrap_or_else(|| "default".to_owned());
...
fn next_default_export_reference(syntax: TypeScriptSyntax<'_, '_>) -> Option<String> {
        ...
        .filter(|node| node.kind() == "export_statement")
    {
        ...
        for child in statement.named_children(&mut cursor) {
            ...
            if matches!(child.kind(), "identifier" | "member_expression") {
                return syntax.text(child).map(str::to_owned);
            }
        }
    }
    None
