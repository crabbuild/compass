...
fn syntax_descriptor(input: &FileInput<'_>) -> ProviderDescriptor {
...
pub(crate) fn extract_from_tree(
    ...
    let source_file = normalize_source_path(source_file)?;
    let input = FileInput {
        source_file: &source_file,
        language,
        source,
    };
    let descriptor = syntax_descriptor(&input);
    let batch = match language {
        "python" => python::extract(descriptor, &input, root),
        "rust" => rust::extract(descriptor, &input, root),
        "typescript" | "tsx" | "javascript" => typescript::extract(descriptor, &input, root),
        _ => {
            return Err(ProviderError::InvalidInput(format!(
                "unsupported tree-sitter Program language: {language}"
            )));
        }
    };
...
impl SyntaxProvider for TreeSitterSyntaxProvider {
    ...
    fn analyze_file(
        ...
        let tree = self
            ...
            .map_err(|error| ProviderError::InvalidInput(error.to_string()))?;
        extract_from_tree(&source_file, spec.name, input.source, tree.root_node()).map(Some)
    }
}
