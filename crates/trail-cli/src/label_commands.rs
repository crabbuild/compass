...
fn generate_labels(
    context: &ClusterLabelContext<'_>,
    communities: &trail_graph::Communities,
    arguments: &LabelArguments,
    selected: Option<&str>,
    custom_providers: &serde_json::Map<String, serde_json::Value>,
    environment: &HashMap<String, String>,
...
pub(super) fn label_help(frontend: Frontend) -> String {
    match frontend {
        Frontend::Trail => "Usage: trail graph label [PATH] [--graph PATH] [--backend NAME] [--model NAME] [--missing-only] [--no-viz] [--resolution N] [--exclude-hubs N] [--max-concurrency N] [--batch-size N] [--min-community-size=N] [--timing]",
