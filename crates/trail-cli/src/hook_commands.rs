...
pub(super) fn command_hook_spawn(frontend: Frontend, args: &[String]) -> Outcome {
    let root = args
        .first()
    ...
    let executable = match std::env::current_exe() {
        ...
        Err(error) => {
            return Outcome::failure(format!(
                "error: graph hook could not resolve the Trail executable: {error}"
            ));
        }
    };
...
pub(super) fn hook_help(frontend: Frontend) -> String {
    match frontend {
        Frontend::Trail => "Usage: trail graph hook [install|uninstall|status]",
