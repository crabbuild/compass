...
use trail_core::default_graph_path;
use trail_prs::{
...
pub(super) fn command_prs(frontend: Frontend, args: &[String]) -> Outcome {
    ...
    if parsed.help {
        return match frontend {
            ...
            Frontend::Trail => Outcome::success(prs_help(frontend)),
        };
    }
    ...
    let render = RenderOptions {
        ...
        command_name: match frontend {
            Frontend::Trail => "trail graph prs",
...
pub(super) fn prs_help(frontend: Frontend) -> String {
    match frontend {
        Frontend::Trail => "Usage: trail graph prs [NUMBER] [--triage] [--worktrees] [--conflicts] [--wrong-base] [--base BRANCH] [--repo OWNER/REPO] [--graph PATH]".to_owned(),
