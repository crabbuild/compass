...
#[test]
fn prs_help_and_trail_namespace_are_compatible() -> Result<(), Box<dyn Error>> {
    let repo = repository_root();
    let directory = tempfile::tempdir()?;
    ...
    let trail = Command::new(env!("CARGO_BIN_EXE_trail"))
        .args(["graph", "prs", "--help"])
        .output()?;
    assert!(trail.status.success());
    assert!(String::from_utf8_lossy(&trail.stdout).contains("trail graph prs"));
    let path = std::env::join_paths([bin, PathBuf::from("/usr/bin"), PathBuf::from("/bin")])?;
    let dashboard = Command::new(env!("CARGO_BIN_EXE_trail"))
        .current_dir(directory.path())
        .env("PATH", path)
        .env("NO_COLOR", "1")
        .args(["graph", "prs", "--base", "v8"])
        .output()?;
    let stdout = String::from_utf8_lossy(&dashboard.stdout);
    assert!(stdout.contains("trail graph prs  ·  base: v8"));
