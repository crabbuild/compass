...
mod tests {
    ...
    fn test_json_flat_object() {
        ...
        let Some(root) = extract(source, "json") else {
            return;
        };
        ...
        let host = root
            .children
            .iter()
            .find(|c| c.key.as_deref() == Some("host"));
    ...
    fn test_toml_flat() {
        ...
        let Some(root) = extract(source, "toml") else {
            return;
        };
        assert!(
            root.children
                .iter()
                .any(|c| c.key.as_deref() == Some("host")),
            "should find 'host'"
        );
        assert!(
            root.children
                .iter()
                .any(|c| c.key.as_deref() == Some("port")),
            "should find 'port'"
        );
    }
    ...
    fn test_csv_rows() {
        let source = "a,b,c\n1,2,3\n";
        let Some(root) = extract(source, "csv") else {
            return;
        };
        assert_eq!(root.kind, DataNodeKind::Sequence);
    ...
    fn test_ini_flat() {
        let source = "host=localhost\nport=8080\n";
        let Some(root) = extract(source, "ini") else {
            return;
        };
    ...
    fn test_unsupported_language_returns_none() {
        ...
        assert!(
            result.is_none(),
            "python should return None for data extraction"
        );
    }
}
