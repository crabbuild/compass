...
fn elixir_definition(
    ...
    let (kind, visibility, has_body) = match node_text(&target, source) {
        ...
        "def" | "defdelegate" | "defguard" => {
            (StructureKind::Function, Some("public".to_string()), true)
        }
    ...
    let name = match kind {
        StructureKind::Module | StructureKind::Interface | StructureKind::Impl => {
            elixir_module_name(node, source)
        }
...
fn elixir_definition_body<'a>(
    ...
    let keywords = elixir_child_by_kind(&args, "keywords").or_else(|| {
        elixir_child_by_kind(&args, "list").and_then(|l| elixir_child_by_kind(&l, "keywords"))
    })?;
...
mod tests {
    ...
    fn test_extract_elixir_module_def_and_defp() {
        ...
        assert!(
            ...
            "should find defp helper; got {names:?}"
        );

        let add = funcs
            .iter()
            .find(|s| s.name.as_deref() == Some("add"))
            .unwrap();
        let helper = funcs
            .iter()
    ...
    fn test_extract_elixir_nested_module() {
        ...
        let f = inner
            .children
            .iter()
            .find(|s| s.kind == StructureKind::Function);
    ...
    fn test_extract_elixir_macros_struct_protocol_impl() {
        ...
        assert!(
            module
                .children
                .iter()
                .any(|s| s.kind == StructureKind::Struct),
        ...
        assert_eq!(
            macros.len(),
            2,
            "both defmacro and defmacrop should be Macro items"
        );
        assert!(
            macros
                .iter()
                .any(|s| s.name.as_deref() == Some("mac")
                    && s.visibility.as_deref() == Some("public"))
        );
        assert!(macros.iter().any(
            |s| s.name.as_deref() == Some("pmac") && s.visibility.as_deref() == Some("private")
        ));

        let protocol = intel
            .structure
            .iter()
            .find(|s| s.kind == StructureKind::Interface);
        ...
        assert!(
            intel
                .structure
                .iter()
                .any(|s| s.kind == StructureKind::Impl),
    ...
    fn test_extract_elixir_quote_block_not_walked() {
        ...
        assert!(
            intel.imports.is_empty(),
            "import inside quote must not be reported"
        );
        ...
        assert!(
            ...
            "def inside quote must not be reported"
        );
    }
