...
pub(crate) fn extract_docstrings(
    root: &tree_sitter::Node,
    source: &str,
    language: &str,
) -> Vec<DocstringInfo> {
...
pub(super) fn collect_structure(
    ...
    let structure_kind = match kind {
        ...
        "module_definition" | "mod_item" | "package_header" | "package_declaration" => {
...
mod tests {
    ...
    fn test_extract_python_function() {
        ...
        assert!(
            !intel.structure.is_empty(),
            "should find at least one structure item"
        );
    ...
    fn test_extract_ruby_module_class_and_methods() {
        ...
        assert!(
            class.is_some(),
            "should find a Ruby class inside the module"
        );
        let class = class.unwrap();
        ...
        let method_names = class
            ...
            .collect::<Vec<_>>();
        assert!(
            method_names.contains(&"call"),
            "should find an instance method"
        );
        assert!(
            method_names.contains(&"build"),
            "should find a singleton method"
        );
    }
    ...
    fn test_metrics_counts() {
        ...
        assert!(
            intel.metrics.total_lines >= 4,
            "should have at least 4 lines"
        );
        assert!(
            intel.metrics.blank_lines >= 1,
            "should have at least 1 blank line"
        );
        assert!(
            intel.metrics.comment_lines >= 1,
            "should have at least 1 comment line"
        );
        assert!(
            intel.metrics.code_lines >= 2,
    ...
    fn test_extract_symbols() {
        ...
        assert!(
            func_symbols.len() >= 2,
            "should find at least 2 function symbols"
        );
    ...
    fn test_error_nodes_detected() {
        ...
        assert!(
            !intel.diagnostics.is_empty(),
            "should have diagnostics for errors"
        );
        assert!(
            intel
                .diagnostics
    ...
    fn collect_structure_kotlin_package_and_class() {
        ...
        assert!(
            module.is_some(),
            "should find a Module entry for the package header"
        );
