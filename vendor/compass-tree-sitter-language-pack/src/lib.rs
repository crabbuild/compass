...
pub use intel::types::{
    ChunkContext, CodeChunk, CommentInfo, CommentKind, DataAttribute, DataNode, DataNodeKind,
    Diagnostic, DiagnosticSeverity, DocSection, DocstringFormat, DocstringInfo, ExportInfo,
    ExportKind, FileMetrics, ImportInfo, ProcessResult, Span, StructureItem, StructureKind,
...
pub use queries::{
    get_folds_query, get_highlights_query, get_indents_query, get_injections_query,
    get_locals_query, get_tags_query,
};
...
#[cfg(feature = "download")]
static CUSTOM_CACHE_DIR: LazyLock<RwLock<Option<std::path::PathBuf>>> =
...
pub fn detect_language(path: &str) -> Option<&'static str> {
    detect_language_from_path(path)
        .or_else(|| detect_language_from_extension(path.trim_start_matches('.')))
}
...
pub fn prefetch(languages: &[&str]) -> Result<(), Error> {
    ...
    let resolved: Vec<&str> = languages
        .iter()
        .map(|n| crate::registry::resolve_alias(n))
        .collect();
...
mod tests {
    ...
    fn test_get_language_and_parse() {
        ...
        for lang_name in &langs {
            ...
            assert!(
                tree.is_some(),
                "Parser for '{lang_name}' should parse a string"
            );
        }
    }
