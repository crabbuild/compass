...
#[cfg(any(
    all(feature = "dynamic-loading", not(target_arch = "wasm32")),
    feature = "download",
))]
#[inline(always)]
...
mod dynamic {
    ...
    fn language_from_process_library(
        ...
        let lib = libraries.get(lib_key).ok_or_else(|| {
            Error::DynamicLoad(format!(
                "Loaded library {} missing from cache",
                lib_path.display()
            ))
        })?;
...
impl LanguageRegistry {
    ...
    pub fn new() -> Self {
        ...
        Self {
            static_lookup,
            #[cfg(feature = "dynamic-loading")]
            dynamic_loader: dynamic::DynamicLoader::new(
                PathBuf::from(LIBS_DIR),
                DYNAMIC_LANGUAGE_NAMES.to_vec(),
            ),
            #[cfg(feature = "dynamic-loading")]
    ...
    pub fn get_language(&self, name: &str) -> Result<Language, Error> {
        ...
        #[cfg(feature = "dynamic-loading")]
        {
            ...
            if self.dynamic_loader.dynamic_names.contains(&name)
                || self.dynamic_loader.lib_file_exists(name)
            {
...
mod tests {
    ...
    fn test_registry_process() {
        let Some(lang) = first_available_lang() else {
            return;
        };
        let registry = LanguageRegistry::new();
    ...
    fn test_registry_process_with_chunking() {
        ...
        assert!(
            result.is_ok(),
            "registry.process() with chunking should succeed"
        );
        let intel = result.unwrap();
        assert_eq!(intel.language, lang);
    ...
    fn test_dynamic_language_survives_registry_drop() {
        ...
        let tree = parser
            .parse("x", None)
            .expect("parser should work after registry drop");
            ...
            for language in &languages {
                scope.spawn(move || {
                    for _ in 0..16 {
                        ...
                        let result =
                            crate::process("x", &config).expect("process should be thread-safe");
