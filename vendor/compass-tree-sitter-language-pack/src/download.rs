...
fn read_file_url(url: &str) -> Result<String, Error> {
    ...
    fs::read_to_string(path)
        .map_err(|e| Error::Download(format!("Failed to read manifest from {url}: {e}")))
}
...
fn sibling_tmp_path(dest: &Path) -> Result<PathBuf, Error> {
    let parent = dest.parent().ok_or_else(|| {
        Error::CacheLock(format!("destination has no parent dir: {}", dest.display()))
    })?;
    let name = dest.file_name().and_then(|s| s.to_str()).ok_or_else(|| {
        Error::CacheLock(format!("destination has no filename: {}", dest.display()))
    })?;
...
impl DownloadCacheLock {
    ...
    pub(crate) fn open(cache_dir: &Path) -> Result<Self, Error> {
        fs::create_dir_all(cache_dir).map_err(|e| {
            Error::CacheLock(format!("create cache dir {}: {e}", cache_dir.display()))
        })?;
        ...
        let file = OpenOptions::new()
            ...
            .open(&lock_path)
            .map_err(|e| {
                Error::CacheLock(format!("open lock file {}: {e}", lock_path.display()))
            })?;
        Ok(Self {
...
impl DownloadManager {
    ...
    #[cfg_attr(alef, alef(skip))]
    pub fn with_cache_dir_and_tls(
        version: &str,
        cache_dir: PathBuf,
        tls_roots: Option<TlsRootsMode>,
    ) -> Self {
    ...
    pub fn default_cache_dir(version: &str) -> Result<PathBuf, Error> {
        let base = dirs::cache_dir().ok_or_else(|| {
            Error::Download("Could not determine system cache directory".to_string())
        })?;
        Ok(base
    ...
    pub fn ensure_languages(&self, names: &[&str]) -> Result<(), Error> {
        ...
        let missing: Vec<&str> = names
            .iter()
            .filter(|name| !self.is_cached(name))
            .copied()
            .collect();
        if missing.is_empty() {
            return Ok(());
    ...
    fn load_verified_cached_bundle(
        cache_path: &Path,
        expected_sha256: &str,
    ...
    fn download_bundle(&self, url: &str) -> Result<Vec<u8>, Error> {
        if let Some(path) = url.strip_prefix("file://") {
            return fs::read(path)
                .map_err(|e| Error::Download(format!("Failed to read bundle from {url}: {e}")));
        }
    ...
    pub(crate) fn extract_languages(
        &self,
        archive_data: &[u8],
        names: &[&str],
    ) -> Result<(), Error> {
        ...
        {
            let mut entry = entry
                .map_err(|e| Error::Download(format!("Failed to read archive entry: {}", e)))?;
            let path = entry
                .path()
            ...
            if expected_files.contains_key(&filename) {
                ...
                atomic_copy_from_reader(&dest, &mut entry).map_err(|e| {
                    Error::Download(format!("Failed to extract {}: {}", filename, e))
                })?;
    ...
    #[cfg_attr(alef, alef(skip))]
    pub fn _testing_extract_languages(
        &self,
        archive_data: &[u8],
        names: &[&str],
    ) -> Result<(), Error> {
...
mod tests {
    ...
    fn bundle_cache_path_uses_version_cache_dir() {
        ...
        assert_eq!(
            ...
            cache_dir
                .parent()
                .unwrap()
                .join("bundles/macos-arm64-abc123.tar.zst")
        );
    }
    ...
    fn verified_bundle_cache_returns_matching_archive_bytes() {
        ...
        fs::create_dir_all(cache_path.parent().unwrap())
            .expect("bundle cache directory should be created");
    ...
    fn verified_bundle_cache_removes_hash_mismatch() {
        ...
        fs::create_dir_all(cache_path.parent().unwrap())
            .expect("bundle cache directory should be created");
        fs::write(&cache_path, b"corrupt archive bytes")
            .expect("bundle cache file should be written");
    ...
    fn extract_languages_writes_requested_library() {
        ...
        let extracted =
            fs::read(manager.lib_path("python")).expect("extracted library should be readable");
    ...
    fn clean_cache_removes_libraries_bundles_and_manifest() {
        ...
        fs::create_dir_all(library_path.parent().unwrap())
            .expect("library cache directory should be created");
        ...
        std::thread::scope(|scope| {
            for _ in 0..8 {
                ...
                scope.spawn(move || {
                    manager
                        .clean_cache()
                        .expect("concurrent cleanup should succeed")
                });
            }
        });
    ...
    fn resolve_tls_roots_returns_explicit_override_when_provided() {
        let _guard = TLS_ENV_GUARD
            .lock()
            .expect("env guard should not be poisoned");
        let _env = EnvVarGuard::set(TLS_ROOTS_ENV, "webpki");
        assert_eq!(
            resolve_tls_roots(Some(TlsRootsMode::Platform)),
            TlsRootsMode::Platform
        );
        assert_eq!(
            resolve_tls_roots(Some(TlsRootsMode::WebPki)),
            TlsRootsMode::WebPki
        );
    }
    ...
    fn resolve_manifest_url_defaults_to_github_release() {
        let _guard = MANIFEST_URL_ENV_GUARD
            .lock()
            .expect("env guard should not be poisoned");
    ...
    fn resolve_manifest_url_honours_env_override_http() {
        ...
        assert_eq!(
            resolve_manifest_url("1.2.3"),
            "https://mirror.example.com/parsers.json"
        );
    }
    ...
    fn read_file_url_errors_on_non_file_scheme() {
        let _guard = MANIFEST_URL_ENV_GUARD
            .lock()
            .expect("env guard should not be poisoned");
        let err = read_file_url("https://example.com/parsers.json")
            .expect_err("non-file URL should error");
        assert!(matches!(err, Error::Download(_)));
    }
    ...
    fn fetch_manifest_reads_from_file_url_when_env_is_set() {
        ...
        let _env = EnvVarGuard::set(
            MANIFEST_URL_ENV,
            &format!("file://{}", manifest_src.display()),
        );
    ...
    fn download_manager_constructor_honours_explicit_tls_override() {
        ...
        for mode in [TlsRootsMode::Platform, TlsRootsMode::WebPki] {
            let dm = DownloadManager::with_cache_dir_and_tls(
                "test",
                temp_dir.path().join("libs"),
                Some(mode),
            );
