...
Inspect the published release and replace autogenerated notes with curated notes titled `Compass v$version`. The expected asset set is 14 files: six `.tar.gz` archives, six matching `.sha256` files, `install.sh`, and `install.ps1`.
...
test "$(find "$verify_dir" -maxdepth 1 -type f | wc -l | tr -d ' ')" = 14
...
- six target archives, six checksums, and the two installer scripts (or the VSIX path/checksum when requested);
