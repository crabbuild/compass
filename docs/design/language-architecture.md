...
| Available now | Rust is a quality-gated, hard-cut version-5 `UniversalCandidate`; version 5 preserves impl-scoped associated types and exact `Self::Type` returns, version 4 publishes scoped type, lifetime, and const generic parameters, and version 3 preserves nested function declarations and lexical calls; replaced publisher and collection resolution branches remain removed |
